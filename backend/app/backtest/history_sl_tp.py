...
def _clamp_sl(sl: float) -> float:
    return round(max(4.0, min(10.0, sl)), 2)


def _clamp_tp(tp: float) -> float:
    return round(max(5.0, min(10.0, tp)), 2)
...
def merge_sl_tp_with_history(
    ...
    *,
    ...
    grid_tp = _clamp_tp(max(grid_tp, grid_sl * 1.15))
    grid_tp = round(min(grid_tp, grid_sl * 1.8), 2)
    ...
    hist_tp = _clamp_tp(max(w_tp / total_w, hist_sl * 1.15))
    hist_tp = round(min(hist_tp, hist_sl * 1.8), 2)
    final_sl = _clamp_sl(grid_sl * 0.65 + hist_sl * 0.35)
    final_tp = _clamp_tp(max(grid_tp * 0.65 + hist_tp * 0.35, final_sl * 1.15))
    final_tp = round(min(final_tp, final_sl * 1.8), 2)
    final_sl = _clamp_sl(final_sl)
    final_tp = _clamp_tp(final_tp)

    notes = [f"그리드 {grid_sl}/{grid_tp}%", f"이력({len(good)}회) {hist_sl}/{hist_tp}%"]
    if trade_total > 0 and end_close_total / trade_total > 0.35:
        final_tp = _clamp_tp(max(final_tp * 0.82, final_sl * 1.15))
        final_tp = round(min(final_tp, final_sl * 1.8), 2)
